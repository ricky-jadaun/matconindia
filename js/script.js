   ...
   "use strict";
    ...
    $(() => {
        ...
        if ($('#menu').length && $.fn.slicknav) {
            if ($('#menu2').length) {
                $('#menu2').children().clone().appendTo('#menu');
            }
            $('#menu').slicknav({
                label: "",
        ...
        // Sticky Navbar On Scroll (Top Zero for all pages)
        const $headerInner = $('#sisf-page-header-inner');
        if ($headerInner.length) {
            ...
            const checkSticky = () => {
                ...
                if (scrollPos > 40) {
                    if (!isSticky) {
                        $headerInner.addClass('is-sticky');
                        isSticky = true;
                    }
                } else {
                    if (isSticky) {
                        $headerInner.removeClass('is-sticky');
                        isSticky = false;
                    }
                }
            };
    ...
    $window.on("resize", () => {
        if ($window.width() > 991 && $.fn.slicknav && $('#menu').length) {
            $('#menu').slicknav('close');
        }
    });
